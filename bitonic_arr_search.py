#  to search an ele in bitonic array , find the peak , then apply bin search in both arrays 
# bin_search_asc(arr,ele , 0 , peak-1)
# bin_search_desc(arr , ele , peak , len(arr)-1)
...
def bin_search_asc(arr , ele , start , end):
    while start<=end:
        mid = start+(end-start)//2
        if arr[mid]==ele:
            return mid
...
def bin_search_desc(arr , ele ,start , end):
    while start<=end:
        mid = start+(end-start)//2
        if arr[mid]==ele:
            return mid
...
def search_ele_in_bitonic_arr(arr,ele):
    peak = peak_ele(arr)
    res1 = bin_search_asc(arr,ele , 0 , peak-1)
    res2 = bin_search_desc(arr , ele , peak , len(arr)-1)
    if res1 !=-1:
        return res1
    else:
        return res2
...
ele = 7
print(search_ele_in_bitonic_arr(arr,ele))
